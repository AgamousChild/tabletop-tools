...
| 10 | Build the **validation process** — admin UI + LLM evaluator path — that gates re-keys before they become active | 🟡 in progress | v3 design (UPDATE-in-place + audit log). **10a** ✅ schema + migration 0005 + tests. **10b/10c** ✅ producer + 11th-ingest script use new schema; divergent canonical queues a candidate (with deploy-order safety check). **Remaining**: 10d admin router, 10e admin UI, 10f LLM evaluator, 10g real-data validation |
