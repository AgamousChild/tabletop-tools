...
- **Phase 3 Versus complete (2026-06-01)** — replaced JSON-blob `simulations` table with three normalized tables (`simulation`, `simulation_weapon`, `simulation_modifier`). `attacker_unit_id`/`defender_unit_id` FK into `list_unit` (nullable, ON DELETE SET NULL). Attack-count invariant enforced server-side (`attackCount.ts`: `resolveAttacksExpected` + `assertAttackCountInvariant`). `simulateV2` tRPC router (save/history/get/delete). Client `useSimulateV2` hook + `weaponAbilityToModifier` exhaustive switch (22 variants). `SimulationHistory` migrated from JSON parsing to real columns. Old `simulations`/`simulate` kept for backward-compat. 99 db tests + 50 server tests + 201 client tests. Playwright e2e smoke tests added (`e2e/specs/versus-v2.spec.ts`). **Migration `0009` applied to prod 2026-06-05** (`apps/data-import/.local/apply-pending-migrations.mjs` workaround — drizzle-kit migrate silently skipped 0009–0012 due to out-of-order journal `when` timestamps).
- **Phase 3 Tournament + BCP complete (2026-06-01)** — 5 new tables: `ranking_metric` (metric catalog), `tournament_pairing_metric` + `tournament_placing_metric` (ordered tiebreaker stacks), `passthrough_event` (BCP event directory), `bcp_registration` (consent-gated submission record). `tournament_players` gains `faction_entity_id`, `detachment_entity_id` (FK→content_entity), `placement`. Metric-stack standings engine (`metric-compute.ts`) replaces hardcoded tiebreaker order. `exportToMeta` freezes `placement` at tournament close. Client: `FactionDetachmentPicker` (data-driven from content_entity), `MetricStackStandings` (columns from metricKeys[]), `PassthroughDirectory`, `BcpListDrop`. 108 db tests + 100 server tests + 58 client tests. **Migration `0010_tournament_phase3` applied to prod 2026-06-05**.
- **Phase 3 Game Tracker v2 complete (2026-06-01)** — relational `match_v2`/`match_player`/`battle_round`/`round_player`/`score_event`/`game_state_event`/`match_secondary_v2`/`unit_casualty`/`unit_state`/`stratagem_use` + `deployment`/`deployment_objective`/`terrain_layout`/`mission_card`/`match_player_primary_option`. **Migration `0011_game_tracker_v2` applied to prod 2026-06-05**.
- **Phase 2 Support attachment migration (2026-06-04)** — `content_can_lead` gains `role` column (PK = leader+bodyguard+role), `content_entity` gains `can_deploy_solo` (default true). **Migration `0012_worthless_agent_brand` applied to prod 2026-06-05**. 1811 existing rows backfilled as role='leader'. Pre-migration DB snapshot: `.local/db-backup-2026-06-05T02-52-54/` (129,185 rows / 67 tables).
- **Migration-history caveat:** drizzle-kit migrate is currently silently skipping any new migration whose journal `when` is lower than the latest applied `created_at`. Journal entries for 0009/0010/0011/0012 were authored with timestamps earlier than 0007/0008 (which were only applied 2026-06-05). Workaround script `.local/apply-pending-migrations.mjs` executes statements via `executeMultiple` and inserts proper `__drizzle_migrations` rows (hash-matched, so future drizzle-kit runs skip them correctly). To restore normal drizzle-kit flow, fix the `when` timestamps in `packages/db/migrations/meta/_journal.json` for any future-generated migrations to be strictly greater than the latest existing entry.
...
| T3 | Apply migration `0012` to prod + re-run data-import sync | ✅ done (2026-06-05) | Migration applied (workaround for stale journal `when`). 1811 rows backfilled as role='leader'. Data-import re-sync still TODO to populate any new content; e2e gate `TEST_ATTACHMENT_DATA=1` unblocked once test data exists. |
