/**
 * Meta analytics cube builder.
 *
 * Transforms 3NF tournament data (meta_events, meta_event_players, meta_pairings)
 * into the analytics cube (meta_for, fact_game_results, meta_top).
 *
 * Ported from apps/content-ingestor/src/meta/build-cube.ts to accept a Drizzle Db
 * instance instead of creating its own libSQL client.
 */
...
export async function runPipeline(db: Db): Promise<void> {
  ...
  try {
    ...
    await db.run(sql`DELETE FROM meta_top`)
    await db.run(sql`DELETE FROM fact_game_results`)
    await db.run(sql`DELETE FROM meta_for`)
    ...
    const events = (await db.all(
      sql`SELECT id, date, name FROM meta_events`,
    ...
    for (const f of frames) {
      await db.run(sql`INSERT OR REPLACE INTO meta_for
    ...
    // 4. Build meta_top — aggregate faction stats per frame
    for (const frame of frames) {
      ...
      const totalPlayers = stats.reduce(
        (sum, r: any) => sum + (r.players as number),
        0,
      )
      ...
      for (const row of stats as any[]) {
        ...
        const playerPct =
          totalPlayers > 0 ? (row.players as number) / totalPlayers : 0
        const overRep = expectedPct > 0 ? playerPct / expectedPct : 0
        const fourOhStart =
          (row.players as number) > 0
