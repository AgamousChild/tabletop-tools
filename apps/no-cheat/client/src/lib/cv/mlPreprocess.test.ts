...
import { rgbaToRgbChw, resizeBilinear, nonMaxSuppression } from './mlPreprocess'
...
describe('nonMaxSuppression', () => {
  function makeDet(x: number, y: number, w: number, h: number, conf: number, cls: number = 0): Detection {
  ...
  it('keeps non-overlapping detections', () => {
    const dets = [
      makeDet(50, 50, 30, 30, 0.9),
      makeDet(200, 200, 30, 30, 0.8),
    ]
    const result = nonMaxSuppression(dets, 0.5)
  ...
  it('preserves class information', () => {
    const dets = [
      makeDet(50, 50, 30, 30, 0.9, 3),
      makeDet(200, 200, 30, 30, 0.8, 5),
    ]
    const result = nonMaxSuppression(dets, 0.5)
