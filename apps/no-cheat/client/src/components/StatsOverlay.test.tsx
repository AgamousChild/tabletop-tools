...
describe('StatsOverlay', () => {
  it('shows dashes when no data', () => {
    render(<StatsOverlay rollCount={0} zScore={null} chiSquared={null} distribution={new Map()} />)
  ...
  it('shows SUSPECT verdict for moderate z-score', () => {
    render(<StatsOverlay rollCount={20} zScore={2.0} chiSquared={8.5} distribution={new Map()} />)
    expect(screen.getByText('SUSPECT')).toBeInTheDocument()
  })

  it('shows LOADED verdict for high z-score', () => {
    render(<StatsOverlay rollCount={50} zScore={3.5} chiSquared={15.2} distribution={new Map()} />)
    expect(screen.getByText('LOADED')).toBeInTheDocument()
  })

  it('shows singular "roll" for count of 1', () => {
    render(<StatsOverlay rollCount={1} zScore={0.1} chiSquared={1.0} distribution={new Map()} />)
