...
const makeUnit = (id: string, name: string, faction: string): UnitProfile => ({
  id,
  faction,
  name,
  move: 6,
...
describe('unit.search', () => {
  it('returns units matching the query', async () => {
    const units = [makeUnit('u1', 'Tactical Squad', 'Space Marines')]
    ...
    expect(mockAdapter.searchUnits).toHaveBeenCalledWith({ name: 'Tactical', faction: undefined })
  })
...
describe('unit.get', () => {
  it('returns the unit when found', async () => {
    const unit = makeUnit('u1', 'Intercessor Squad', 'Space Marines')
    vi.mocked(mockAdapter.getUnit).mockResolvedValueOnce(unit)
    const caller = createCaller(ctx)
    const result = await caller.unit.get({ id: 'u1' })
    expect(result).toMatchObject({ name: 'Intercessor Squad' })
  })

  it('throws NOT_FOUND when unit does not exist', async () => {
    vi.mocked(mockAdapter.getUnit).mockResolvedValueOnce(null)
    const caller = createCaller(ctx)
    await expect(caller.unit.get({ id: 'nonexistent' })).rejects.toMatchObject({
      code: 'NOT_FOUND',
    })
  })
