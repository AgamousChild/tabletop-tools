 ...
 *
 * Shared between brain server (game-data parser) and client (card rendering).
 */
export function deriveUnitType(keywords) {
    const kw = new Set(keywords.map(k => k.toLowerCase()));
    const has = (k) => kw.has(k);
    if (has('epic hero'))
        return 'Epic Hero';
    if (has('imperial knights'))
        return 'Imperial Knight';
    if (has('chaos knights'))
        return 'Chaos Knight';
    ...
    if (has('vehicle'))
        return 'Vehicle';
    if (has('beast') || has('beasts'))
        return 'Beast';
    if (has('fortification'))
        return 'Fortification';
    if (has('mounted') && !has('character'))
        return 'Mounted';
    if (has('battleline'))
        return 'Battleline';
    if (has('swarm'))
