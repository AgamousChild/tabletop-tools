/**
 * Upload the 6 cropped 11e deployment-zone PNGs to R2.
 *
 * Reads from `.local/brain-input/cards/deployment-zone-images/page-{1..6}.png`
 * (output of `scripts/crop-deployment-zone-images.ts`) and pushes each to
 * `pages/ca11-deployment-zones/page-N.png` in the `tabletop-tools-brain`
 * bucket.
 *
 * The `pages/ca11-deployment-zones/` prefix matches the slugified source
 * title that `lib/parsers/deployment-zones.ts::buildDeploymentZoneNodes`
 ...
 *
 * Per Rule 4, the work is exposed as `uploadDeploymentZoneImages()` for use
...
export const DEFAULT_R2_PREFIX = 'pages/ca11-deployment-zones'
export const DEFAULT_LOCAL_DIR = '.local/brain-input/cards/deployment-zone-images'
...
export interface UploadOptions {
  ...
  bucket?: string
  /** R2 key prefix (default: `pages/ca11-deployment-zones`). */
  r2Prefix?: string
  /** Local directory containing `page-{1..6}.png` (default: `.local/...`). */
  localDir?: string
  /** Page count (default: 6, the fixed-size 11e deployment-zone deck). */
  pageCount?: number
  /**
 ...
 */
export function uploadDeploymentZoneImages(opts: UploadOptions = {}): UploadResult {
  ...
  for (let page = 1; page <= pageCount; page++) {
    ...
    if (!existsSync(localPath)) {
      log(`  MISSING page-${page}.png (run crop-deployment-zone-images.ts first)`)
...
function main(): void {
  console.log(`Uploading 11e deployment-zone PNGs → r2://${DEFAULT_BUCKET}/${DEFAULT_R2_PREFIX}/\n`)
  const result = uploadDeploymentZoneImages()
