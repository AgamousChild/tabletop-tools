...
describe('stripFlavorText', () => {
  ...
  it('removes long narrative lines with no numbers or brackets', () => {
    const long =
      'This is a very long narrative flavor text line that goes on and tells a story about some ancient warrior'
  ...
  it('keeps long lines that contain numbers', () => {
    const long =
      'This unit can advance and still shoot with a -1 to hit modifier applied to all attacks made'
    const result = stripFlavorText(long)
  ...
  it('keeps long lines that contain brackets', () => {
    const long =
      'This unit has the following special ability [IGNORES COVER] when targeting units in terrain features'
    const result = stripFlavorText(long)
