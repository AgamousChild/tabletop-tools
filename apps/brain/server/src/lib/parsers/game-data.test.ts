...
describe('convertGameData', () => {
  it('converts datasheets to unit/datasheet nodes', () => {
    const input = makeInput({
      datasheets: [{
        ...
        factionId: 'SM',
        role: 'Battleline',
        legend: '', transport: '', loadout: 'Standard',
        damagedW: '', damagedDescription: '',
      }],
      datasheetModels: [{
  ...
  it('converts weapons to unit/weapon nodes with part_of refs', () => {
    const input = makeInput({
      datasheets: [{
        id: 'abc-123', name: 'Intercessor Squad', factionId: 'SM',
        role: 'Battleline', legend: '', transport: '', loadout: '',
        damagedW: '', damagedDescription: '',
      }],
      datasheetWargear: [{
        id: 1, datasheetId: 'abc-123', name: 'Bolt Rifle',
        description: 'Standard bolt weapon', range: '30"', type: 'Ranged',
  ...
  it('converts unit abilities with part_of refs', () => {
    const input = makeInput({
      ...
      unitAbilities: [{
        id: 'ua1', datasheetId: 'abc-123', name: 'Oath of Moment',
  ...
  it('converts army rules (abilities) to faction/faction-ability nodes', () => {
    const input = makeInput({
      abilities: [{
        id: 'ab-001', name: 'Oath of Moment', legend: '',
        factionId: 'SM',
        description: 'At the start of your Command phase, select one enemy unit. Re-roll hit and wound rolls of 1 against that unit.',
      }],
    })
  ...
  it('converts detachments with abilities, stratagems, and enhancements', () => {
    const input = makeInput({
      detachments: [{
        id: 'det-1', factionId: 'SM', name: 'Gladius Task Force',
        legend: 'The classic Space Marine detachment.', type: 'Standard',
      }],
      detachmentAbilities: [{
        id: 'da-1', detachmentId: 'det-1', factionId: 'SM',
        name: 'Combat Doctrines', legend: '',
        description: 'At the start of your Command phase, you can select a Combat Doctrine.',
      }],
      stratagems: [{
        id: 'str-1', factionId: 'SM', detachmentId: 'det-1',
        name: 'Armour of Contempt', type: 'Battle Tactic',
        cpCost: '1CP', turn: 'Either', phase: 'Shooting/Fight',
        legend: '', description: 'Worsen the AP of attacks against your unit by 1.',
      }],
      enhancements: [{
        id: 'enh-1', factionId: 'SM', detachmentId: 'det-1',
        name: 'Artificer Armour', legend: '',
  ...
  it('creates leader attachment refs', () => {
    const input = makeInput({
      datasheets: [
        { id: 'leader-1', name: 'Captain', factionId: 'SM', role: 'Character', legend: '', transport: '', loadout: '', damagedW: '', damagedDescription: '' },
        { id: 'unit-1', name: 'Intercessors', factionId: 'SM', role: 'Battleline', legend: '', transport: '', loadout: '', damagedW: '', damagedDescription: '' },
      ],
      leaderAttachments: [{
        id: 'la-1', leaderId: 'leader-1', attachedId: 'unit-1',
      }],
    })
  ...
  it('extracts weapon keywords from description', () => {
    const input = makeInput({
      ...
      datasheetWargear: [{
        id: 1, datasheetId: 'abc', name: 'Plasma Gun',
        description: '[RAPID FIRE 1, HAZARDOUS, MELTA 2]', range: '24"', type: 'Ranged',
        attacks: '1', skill: '3+', strength: '7', ap: '-2', damage: '1',
      }],
    })
