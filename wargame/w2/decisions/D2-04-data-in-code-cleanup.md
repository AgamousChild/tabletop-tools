...
| # | Location | What | Size |
|---|---|---|---|
| 1 | `apps/admin/client/src/pages/TasksPage.tsx:1-191` | `TASKS` array — 27 project-tracker items (id/subject/category/priority/status) *(count corrected 28→27, 2026-07-07 hardening pass)* | 27 rows |
...
| 6 | `apps/list-builder/client/src/components/ListBuilderScreen.tsx:56-63` | `byName` — 3rd restatement. *Corrected 2026-07-07 hardening pass: the "inconsistent with #4 on Strike Force maxDuplicates" claim is stale — copies have matched since 2026-06-01 per `git blame`. The duplication stands; the live bug does not. The real divergence is cross-app: bcp-scraper's `bs-parser.ts:51-58` uses a semantically different name→points mapping (see D2-07 item 6).* | 3rd copy |
...
| 11 | `apps/data-import/server/src/lib/sources/bsdata.ts:48-60` | `SM_CHAPTER_TO_SUBFACTION` *(path corrected 2026-07-07: `sources/` segment was missing)* | small |
| 12 | `apps/data-import/server/src/lib/sources/faction-pack.ts:98-129` | `factionSlugToBsdataName` — same underlying fact, 4th variant *(path corrected 2026-07-07)* | 4th copy |
...
| 18 | `apps/brain/server/src/lib/parsers/mission-card-urls.ts` | `PRIMARY_DECKS`/`PRIMARY_CARDS_BY_DECK`/`SECONDARY_CARDS` — OCR-source slug inventory for CA mission cards *(added 2026-07-07 hardening pass — census miss; same class-C cluster as #13/#14, same late-June ingestion push)* | 165 lines |
| 19 | `apps/brain/server/src/lib/parsers/twists.ts` | `TWIST_SLUGS` + per-card content — hand-transcribed CA twist deck; its own comment invokes Rule 6 *(added 2026-07-07)* | 411 lines |
| 20 | `apps/brain/server/src/lib/parsers/secondary-mission-bodies.ts` | 18-card slug/body map for secondary mission text *(added 2026-07-07)* | 266 lines |
...
| 4–7 | list-builder battle-size table ×3 + `ROLE_FILTERS` | A (battle sizes) / borderline A-D (roles) | **Move battle sizes; consolidate roles.** *Corrected 2026-07-07 hardening pass:* the previously-cited internal inconsistency (copy #6 vs #4 on Strike Force maxDuplicates) is stale — the list-builder copies have matched since 2026-06-01. The Rule 6 cost is still concrete, but the live divergence is **cross-app**: bcp-scraper's `bs-parser.ts:51-58` maps names→points on different tier boundaries than list-builder's table ("Strike Force" ≤2000 vs the 1000/2000 split), so the same list gets a different battle-size label depending on which app touches it. `ROLE_FILTERS` is a UI-only derived taxonomy (short, stable, one consumer) — move it too, but lower priority. | New `battle_size` table (or `content_entity`-adjacent lookup) in `packages/db`, one shared read in `game-data-store` used by all 4 list-builder/bcp-scraper call sites (4th copy: `useListsV2.ts:271-276`). `ROLE_FILTERS` → same table family or a `content_entity` category enum if one exists; otherwise defer to D2-07 (shared-utility consolidation) if it turns out UI-only. |
...
| 18–20 | brain `mission-card-urls.ts` / `twists.ts` / `secondary-mission-bodies.ts` (165/411/266 lines) | C | **Same disposition as #13/#14** — added by the 2026-07-07 hardening pass. All three landed in the same 2026-06-28/29 CA ingestion push (`8ac615a`, `f25a43f`) and share the identical class-C shape: hand-transcribed fixed-size deck content with no parse source. The #13/#14 flip trigger covers the whole 5-item cluster. | No change now. Same flip trigger as #13. |
...
2. **Next (ties to D2-01/D2-07):** tournament `MISSIONS` → `scoringMission`
   ...
   `round.ts` isn't touched twice (#2); battle-size consolidation across
   list-builder + bcp-scraper, since the cross-app mapping divergence is a
   live correctness issue (#4-6, corrected 2026-07-07); bcp-scraper
   generated-fallback file (#8); data-import alias-table consolidation into
   one module (#9-12).
3. **Accept, revisit on trigger:** brain's hand-transcribed CA content —
   the full 5-item cluster (#13, #14, #18–20) — no action until a parser
   exists.
